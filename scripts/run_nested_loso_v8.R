#!/usr/bin/env Rscript
# ============================================================
# Nested LOSO M2 Grid Search — v8 (clean baseline + boundary expansion)
#
# Key differences from v7:
#   - Removes clamping (z_ema, d1, d2, logN) from LOSO eval so that the
#     only differences from v4 are (1) weekly refit and (2) soft cap.
#     Clamping stays in pipeline_runtime.R for deployment only.
#   - Expands k_f grid to {2,3,4,5,6,7,8} (v7 best was k_f=6, the max).
#   - k_e stays {2,3,4,6}; k_e=2 is the true minimum non-trivial smooth.
#   - All other settings identical to v7:
#       Kr{1,2,3} x k_f{2,3,4,5,6,7,8} x k_e{2,3,4,6}
#       x alpha{0.01,0.02,...,0.25} = 756 specs
#   - M1-based logit_f_eff (m1_train_preds passed from fold cache).
#   - Reuses v5 Phase 1 M1 cache.
#
# Output: data/nested_loso_v8_phase2.rds (resumable)
#         data/nested_loso_v8_production.rds (final)
# ============================================================

cat("=== Nested LOSO M2 grid search (v8 — clean + k_f expanded) ===\n")
...
# ---- 3. Full v8 spec grid ----
grid_v8 <- tidyr::crossing(
  Kr          = c(1L, 2L, 3L),
  k_f         = c(2L, 3L, 4L, 5L, 6L, 7L, 8L),
  k_e         = c(2L, 3L, 4L, 6L),
  alpha_state = c(0.01, 0.02, 0.03, 0.05, 0.07, 0.10, 0.15, 0.20, 0.25)
)

specs_v8 <- purrr::pmap(grid_v8, function(Kr, k_f, k_e, alpha_state) {
  stage2_make_spec(
    delta       = 0L,
    Kr          = Kr,
    T           = "S",
...
names(specs_v8) <- sprintf(
  "Kr%d_kf%d_ke%d_a%.0f",
  grid_v8$Kr, grid_v8$k_f, grid_v8$k_e, grid_v8$alpha_state * 100
)
cat("Full v8 grid:", length(specs_v8), "specs\n")
...
# v7 used clamping in LOSO — cannot reuse; evaluate all specs fresh.
cv_results_v7 <- list()
new_spec_ids  <- names(specs_v8)
cat("Total v8 specs to evaluate:", length(new_spec_ids), "\n\n")

phase2_ckpt <- "data/nested_loso_v8_phase2.rds"
...
# ============================================================
# Assemble v8 results and report
# ============================================================
cat("\n=== Assembling v8 results ===\n")

cv_results_all <- cv_results_new[names(specs_v8)]
...
saveRDS(results, "data/nested_loso_v8_production.rds")
