#!/usr/bin/env Rscript
# ============================================================
# Nested LOSO M2 Grid Search — v12 (dz_ema covariate + level-only Holt)
#
# Changes from v11:
#   - Removed k_1/k_2 (derivative terms, were fixed at 0)
#   - Added dz_ema = z_ema[t] - z_ema[t-1] as new covariate (k_de param)
#   - Holt EMA simplified to level-only, alpha raised 0.3 → 0.4
#   - Grid: delta × Kr × k_f × k_e × alpha_state × k_r × k_de (72 specs)
#   - k_de=0 is ablation baseline
#
# Output: data/nested_loso_v12_phase2.rds (resumable)
#         data/nested_loso_v12_production.rds (final)
# ============================================================

cat("=== Nested LOSO M2 grid search (v12 — dz_ema + level-only Holt) ===\n")
...
specs_v12 <- purrr::pmap(grid_v12, function(delta, Kr, k_f, k_e, alpha_state, k_r, k_de) {
  stage2_make_spec(
...
names(specs_v12) <- paste0(
  "d+", grid_v12$delta,
  "_Kr", grid_v12$Kr,
  "_kf", grid_v12$k_f,
  "_ke", grid_v12$k_e,
  "_as", grid_v12$alpha_state,
  "_kr", grid_v12$k_r,
  "_kde", grid_v12$k_de
)

cat("Grid size:", length(specs_v12), "specs\n\n")
...
# Save Phase 1 cache
p1_save <- "data/nested_loso_v12_phase1.rds"
...
if (Sys.getenv("SMOKE_TEST", unset = "0") == "1") {
  ...
  fc        <- m1_cache[[test_s]]
  smoke_spec <- specs_v12[[grep("_kde3$", names(specs_v12))[1]]]
  cat("Test season:", test_s, " | Spec:", names(specs_v12)[grep("_kde3$", names(specs_v12))[1]], "\n")
...
# ============================================================
# PHASE 2: Evaluate all v12 specs (frozen GAM + level-only bias)
# ============================================================
cat("=== PHASE 2: M2 grid search (v12 — frozen + level-only bias) ===\n")

new_spec_ids <- names(specs_v12)
cat("Total v12 specs to evaluate:", length(new_spec_ids), "\n\n")

phase2_ckpt <- "data/nested_loso_v12_phase2.rds"
...
# ============================================================
# Assemble v12 results and report
# ============================================================
cat("\n=== Assembling v12 results ===\n")

cv_results_all <- cv_results_new[names(specs_v12)]
...
saveRDS(results, "data/nested_loso_v12_production.rds")

cat("\nResults saved to data/nested_loso_v12_production.rds\n")
