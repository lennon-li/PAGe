#!/usr/bin/env Rscript
# ============================================================
# Nested LOSO M2 Grid Search — v7 (M1-based logit_f_eff, corrected)
#
# Key differences from v6:
#   - Passes real m1_train_preds to nested_loso_m2_eval_weekly_refit()
#     so logit_f_eff in the weekly refit training data is M1-based,
#     matching deployment (pipeline_runtime.R now passes kit$m1_train_preds
#     to refit_stage2_weekly).
#   - v6 used m1_train_preds=NULL (template-based), which was inconsistent
#     with the frozen production GAM (also now M1-based after the fix).
#   - No data leakage: m1_train_preds covers only the LOSO training
#     seasons for each fold; the held-out season is never included.
#   - Same expanded grid as v6:
#       Kr{1,2,3} x k_f{2,3,4,5,6} x k_e{2,3,4,6}
#       x alpha{0.01,0.02,...,0.25} = 540 specs
#   - Reuses v5 Phase 1 M1 cache (M1 walk-forward unchanged).
#   - All 540 specs evaluated fresh (v6 phase2 is stale — used NULL m1_preds).
#
# Output: data/nested_loso_v7_phase2.rds (resumable)
#         data/nested_loso_v7_production.rds (final)
# ============================================================

cat("=== Nested LOSO M2 grid search (v7 — M1-based logit_f_eff) ===\n")
...
grid_v7 <- tidyr::crossing(
  ...
  k_e         = c(2L, 3L, 4L, 6L),
  alpha_state = c(0.01, 0.02, 0.03, 0.05, 0.07, 0.10, 0.15, 0.20, 0.25)
)

specs_v7 <- purrr::pmap(grid_v7, function(Kr, k_f, k_e, alpha_state) {
  stage2_make_spec(
    delta       = 0L,
    Kr          = Kr,
    T           = "S",
...
names(specs_v7) <- sprintf(
  "Kr%d_kf%d_ke%d_a%.0f",
  grid_v7$Kr, grid_v7$k_f, grid_v7$k_e, grid_v7$alpha_state * 100
)
cat("Full v7 grid:", length(specs_v7), "specs\n")
...
# ============================================================
# PHASE 2: Evaluate all specs fresh (v6 results stale — used NULL m1_train_preds)
# ============================================================
cat("=== PHASE 2: M2 grid search (all specs fresh) ===\n")

# v6 used m1_train_preds=NULL — cannot reuse; evaluate all specs fresh.
cv_results_v6 <- list()
new_spec_ids  <- names(specs_v7)
cat("Total v7 specs to evaluate:", length(new_spec_ids), "\n\n")

phase2_ckpt <- "data/nested_loso_v7_phase2.rds"
...
if (length(todo_spec_ids) > 0) {
    ...
    batch_results <- furrr::future_map(
      ...
      function(spec_id) {
        ...
        for (test_s in test_seasons) {
          ...
          # nested_loso_m2_eval_weekly_refit refits internally each week.
          # m1_train_preds passes fold-specific M1 walk-forward predictions
          # so logit_f_eff is M1-based, matching deployment after the
          # pipeline_runtime.R fix.  No leakage: m1_train covers only
          # the LOSO training seasons for this fold.
          eval_out <- tryCatch(
  ...
  cat("All new specs already evaluated — skipping.\n\n")
}

# ============================================================
# Assemble v7 results and report
# ============================================================
cat("\n=== Assembling v7 results ===\n")

cv_results_all <- cv_results_new[names(specs_v7)]
...
saveRDS(results, "data/nested_loso_v7_production.rds")
