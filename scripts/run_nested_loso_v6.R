#!/usr/bin/env Rscript
# ============================================================
# Nested LOSO M2 Grid Search — v6 (expanded grid)
#
# Key differences from v5:
#   - Expanded grid covers boundaries found in v5:
#       k_f extended to {2,3,4,5,6}   (was {2,3,4}; v5 best was at k_f=4 max)
#       k_e extended to {2,3,4,6}     (was {3,4,6}; v5 best was at k_e=3 min)
#       alpha extended to {0.01,0.02,...,0.25} (was {0.03,...}; v5 best at 0.03 min)
#   - Reuses v5 Phase 1 M1 cache (M1 walk-forward unchanged)
#   - Reuses v5 Phase 2 results for the 189 specs already evaluated
#   - Only computes the 351 NEW specs not covered by v5
#   - Full grid: Kr{1,2,3} x k_f{2,3,4,5,6} x k_e{2,3,4,6}
#                x alpha{0.01,0.02,0.03,0.05,0.07,0.10,0.15,0.20,0.25}
#                = 540 specs total
#
# Output: data/nested_loso_v6_phase2.rds (resumable, new specs only)
#         data/nested_loso_v6_production.rds (merged v5+v6, final)
# ============================================================

cat("=== Nested LOSO M2 grid search (v6 — expanded grid) ===\n")
...
specs_v6 <- purrr::pmap(grid_v6, function(Kr, k_f, k_e, alpha_state) {
  stage2_make_spec(
    delta       = 0L,
    Kr          = Kr,
    T           = "S",
...
names(specs_v6) <- sprintf(
  "Kr%d_kf%d_ke%d_a%.0f",
  grid_v6$Kr, grid_v6$k_f, grid_v6$k_e, grid_v6$alpha_state * 100
)
cat("Full v6 grid:", length(specs_v6), "specs\n")
...
# ============================================================
# PHASE 2: Evaluate NEW specs only; reuse v5 for existing ones
# ============================================================
cat("=== PHASE 2: M2 grid search (new specs only) ===\n")

# v5 results used d1_now=0 (bug fixed in v6) — cannot reuse; evaluate all specs fresh.
cv_results_v5 <- list()
new_spec_ids  <- names(specs_v6)
cat("Total v6 specs to evaluate:", length(new_spec_ids), "\n\n")

phase2_ckpt <- "data/nested_loso_v6_phase2.rds"
    ...
    batch_results <- furrr::future_map(
      ...
      function(spec_id) {
        ...
        for (test_s in test_seasons) {
          ...
          # no separate nested_loso_m2_train call needed.
          eval_out <- tryCatch(
            nested_loso_m2_eval_weekly_refit(
              ...
              spec           = spec,
              m1_train_preds = NULL,  # NULL = template-based logit_f_eff, matching deployment
              eval_window    = 12L,
...
# ============================================================
# Assemble v6 results and report
# ============================================================
cat("\n=== Assembling v6 results ===\n")

cv_results_all <- cv_results_new[names(specs_v6)]
...
saveRDS(results, "data/nested_loso_v6_production.rds")
