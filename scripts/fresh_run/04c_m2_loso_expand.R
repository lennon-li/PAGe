#!/usr/bin/env Rscript
# Step 4c — M2 Boundary Expansion (fresh run)
#
# Reads data/fresh_nested_loso_v15_postfix_production.rds, checks whether
# the best spec lands on any boundary of the current grid, and runs additional
# specs for each boundary dimension.  Iterates until no boundary hit or no
# improvement.
#
# Reads:   data/fresh_nested_loso_v15_postfix_production.rds
#          data/fresh_nested_loso_v15_phase1.rds
# Updates: data/fresh_nested_loso_v15_postfix_production.rds  (merged)
#          data/fresh_nested_loso_v15_postfix_phase2_ckpt.rds (merged)

source("scripts/fresh_run/00_shared.R")
cat("=== Step 4c: M2 Boundary Expansion ===\n")
cat("Start:", format(Sys.time()), "\n\n")

# ---- Load current results ----
results_path <- "data/fresh_nested_loso_v15_postfix_production.rds"
ckpt_path    <- "data/fresh_nested_loso_v15_postfix_phase2_ckpt.rds"

if (!file.exists(results_path)) stop("Run 04_m2_loso_postfix.R first.")
res          <- readRDS(results_path)
cv_results   <- res$cv_results   # named list: spec_id -> list(scores, predictions)
...
best_nll <- function(df, dim_name, val) {
  col <- as.numeric(sub(
    paste0(".*_", dim_name, "([0-9.]+).*"), "\\1", df$spec_id))
  min(df$bernoulli_nll[col == val], na.rm = TRUE)
}
...
# Dimension definitions: name in spec_id, grid col name, step size, hard limits
DIMS <- list(
  list(id = "kf",  col = "k_f",         step = 1L,    lo = 2L,   hi = 12L,
       to_int = TRUE),
  list(id = "ke",  col = "k_e",         step = 1L,    lo = 2L,   hi = 6L,
       to_int = TRUE),
  list(id = "as",  col = "alpha_state",  step = 0.05,  lo = 0.20, hi = 0.70,
       to_int = FALSE),
  list(id = "ksp", col = "k_sp",        step = 2L,    lo = 0L,   hi = 8L,
       to_int = TRUE),
  list(id = "ba",  col = "bias_alpha",   step = 0.1,   lo = 0.1,  hi = 1.0,
       to_int = FALSE)
)
...
for (round_i in seq_len(MAX_ROUNDS)) {
  ...
  for (d in DIMS) {
    ...
    if (bc$at_lower && bc$best_val - d$step >= d$lo) {
      ...
      extend_vals <- new_val
      cat(sprintf("  LOWER boundary → adding %s", new_val))
    }
    if (bc$at_upper && bc$best_val + d$step <= d$hi) {
      new_val <- if (d$to_int) as.integer(bc$best_val + d$step)
                 else round(bc$best_val + d$step, 6)
      extend_vals <- c(extend_vals, new_val)
      cat(sprintf("  UPPER boundary → adding %s", new_val))
    }
    cat("\n")

    if (length(extend_vals) > 0) {
      # New rows: extend this dimension, keep all other dims at current ranges
  ...
  # Drop already-evaluated specs
  new_grid$spec_id_tmp <- purrr::pmap_chr(new_grid, function(...) {
    spec_id_from_row(list(...))
  })
  ...
  # Extend grid_current with new dimension values
...
# ---- Comparison vs gold ----
cat("\n=== Comparison vs gold (nested_loso_v15_postfix_production.rds) ===\n")
gold     <- readRDS("data/nested_loso_v15_postfix_production.rds")
gold_sum <- gold$summary |> dplyr::arrange(bernoulli_nll)

cat("Gold best (pre-L2):", gold_sum$spec_id[1], "| NLL:", round(gold_sum$bernoulli_nll[1], 5), "\n")
cat("Fresh best (L2-fix):", best_id,             "| NLL:", round(summary_df$bernoulli_nll[1], 5), "\n")
cat("Best spec match:", if (gold_sum$spec_id[1] == best_id) "YES" else
    sprintf("NO — gold=%s  fresh=%s", gold_sum$spec_id[1], best_id), "\n")

# Per-spec delta (only specs in both)
...
cat(sprintf("Matched specs: %d  |  mean delta: %.5f  |  max |delta|: %.5f\n",
            nrow(cmp), mean(cmp$delta, na.rm = TRUE),
            max(abs(cmp$delta), na.rm = TRUE)))
cat("(Negative delta expected: L2 fix lowers NLL ~0.016)\n")
