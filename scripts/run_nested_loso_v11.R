#!/usr/bin/env Rscript
# ============================================================
# Nested LOSO M2 Grid Search — v11 (simplified model + z_resid + bias correction)
#
# Changes from v9/v10:
#   - Dropped terms: logN_now (k_n=0), newWeek (k_w=0), factor-spline (k_s=0),
#     derivative (k_1=0). These were consistently disabled by LOSO tuning.
#   - Added z_resid = z_ema - logit_f_eff as amplitude residual covariate (k_r).
#   - Added Kalman-style running bias correction (EMA of logit residuals).
#     Bias correction is applied during walk-forward evaluation, matching
#     the deployment pipeline.
#   - Grid: delta × Kr × k_f × k_e × alpha_state × k_r (216 specs).
#
# Output: data/nested_loso_v11_phase2.rds (resumable)
#         data/nested_loso_v11_production.rds (final)
# ============================================================

cat("=== Nested LOSO M2 grid search (v11 — simplified + z_resid + bias) ===\n")
...
# ---- 3. v11 simplified spec grid ----
# Dropped: k_n, k_w, k_s, k_1 (all fixed at 0 by v9/v10 tuning)
# Added: k_r (amplitude residual z_resid = z_ema - logit_f_eff)
# Fixed: k_2=0, lambda_w=0, T="S", anchorWeek=20
grid_v11 <- tidyr::crossing(
  delta       = c(-1L, 0L, 1L),
  Kr          = c(1L, 2L, 3L),
  k_f         = c(3L, 5L, 7L),
  k_e         = c(2L, 3L, 4L),
...
cat("Raw grid size:", nrow(grid_v11), "\n")

# Build spec objects
specs_v11 <- purrr::pmap(grid_v11, function(delta, Kr, k_f, k_e, alpha_state, k_r) {
  stage2_make_spec(
    delta       = delta,
    Kr          = Kr,
    T           = "S",
...
names(specs_v11) <- sprintf(
  "d%+d_Kr%d_kf%d_ke%d_a%.0f_kr%d",
  grid_v11$delta, grid_v11$Kr, grid_v11$k_f, grid_v11$k_e,
  grid_v11$alpha_state * 100, grid_v11$k_r
)
cat("Full v11 grid:", length(specs_v11), "specs\n")
...
# Save Phase 1 cache
p1_save <- "data/nested_loso_v11_phase1.rds"
...
# ============================================================
# PHASE 2: Evaluate all v11 specs (frozen GAM + trend-augmented bias)
# ============================================================
cat("=== PHASE 2: M2 grid search (v11 — frozen + bias) ===\n")

new_spec_ids <- names(specs_v11)
cat("Total v11 specs to evaluate:", length(new_spec_ids), "\n\n")

phase2_ckpt <- "data/nested_loso_v11_phase2.rds"
    ...
    batch_results <- furrr::future_map(
      ...
      function(spec_id) {
        ...
        for (test_s in test_seasons) {
          ...
          # Train frozen GAM once per fold
          m2_fit <- tryCatch(
          ...
          # Evaluate with frozen GAM + trend-augmented bias correction
          eval_out <- tryCatch(
...
# ============================================================
# Assemble v11 results and report
# ============================================================
cat("\n=== Assembling v11 results ===\n")

cv_results_all <- cv_results_new[names(specs_v11)]
...
saveRDS(results, "data/nested_loso_v11_production.rds")

cat("\nResults saved to data/nested_loso_v11_production.rds\n")
