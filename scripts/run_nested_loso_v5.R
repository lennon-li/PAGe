#!/usr/bin/env Rscript
# ============================================================
# Nested LOSO M2 Grid Search — v5 (weekly refit + soft cap)
#
# Key differences from v4:
#   - Evaluation uses nested_loso_m2_eval_weekly_refit() instead
#     of frozen-fit nested_loso_m2_eval().
#   - Soft cap (make_soft_cap_fn) is applied consistently inside
#     the weekly-refit eval loop.
#   - Reuses Phase 1 M1 cache from v4 to avoid recomputing M1
#     walk-forward (only M2 evaluation semantics change).
#
# Grid: same as v4
#   Kr in {1,2,3} x k_f in {2,3,4} x k_e in {3,4,6}
#   x alpha in {0.03,0.05,0.07,0.10,0.15,0.20,0.25}
#   = 189 specs
#
# Output: data/nested_loso_v5_phase2.rds (resumable)
#         data/nested_loso_v5_production.rds (final)
# ============================================================

cat("=== Nested LOSO M2 grid search (v5 — weekly refit + soft cap) ===\n")
...
# Source development copies so we pick up unreleased edits without
# requiring a full package reinstall.
for (f in c(
  "R/utils.R",
...
# ============================================================
# PHASE 1: Reuse v4 M1 cache (only M2 eval changes in v5)
# ============================================================
cat("=== PHASE 1: Loading M1 cache ===\n")

phase1_v5 <- "data/nested_loso_v5_phase1.rds"

for (src in c(
  phase1_v5,
  "data/nested_loso_v4_phase1.rds",
  "data/archive/nested_loso_v4_phase1.rds",
  "data/archive/nested_loso_v3_phase1.rds"
)) {
  if (file.exists(src)) {
...
if (length(remaining_p1) > 0) {
  cat("Computing", length(remaining_p1), "missing folds for Phase 1...\n")
...
# Save v5 copy if needed
if (!file.exists(phase1_v5)) saveRDS(m1_cache, phase1_v5)

cat("Phase 1 complete:", length(m1_cache), "folds.\n\n")

# ============================================================
# PHASE 2: M2 grid search with weekly refit eval
# ============================================================
cat("=== PHASE 2: M2 grid search (weekly refit eval) ===\n")
cat("(", length(specs), "specs x", length(test_seasons), "folds)\n\n")

phase2_ckpt <- "data/nested_loso_v5_phase2.rds"
  ...
  batch_results <- furrr::future_map(
    ...
    function(spec_id) {
      ...
      for (test_s in test_seasons) {
        ...
        # Evaluate with weekly refit (v5 key change)
        eval_out <- tryCatch(
...
saveRDS(results, "data/nested_loso_v5_production.rds")
