#!/usr/bin/env Rscript
# ============================================================
# Optimized Nested LOSO M2 Grid Search — v2
#
# KEY SPEEDUP vs v1: M1 walk-forward is computed ONCE per fold,
# then reused across all 48 M2 specs. This eliminates a 48x
# redundant re-computation that caused the 3-day runtime.
#
# Architecture:
#   Phase 1 — Build all folds + compute M1 predictions (runs ONCE)
#              Parallelized: future::multicore across seasons within each fold
#              Checkpoint: data/nested_loso_v2_phase1.rds
#
#   Phase 2 — M2-only grid search over 48 specs (fast: seconds per fold)
#              Parallelized: furrr::future_map across specs
#              Checkpoint: data/nested_loso_v2_phase2.rds
#
# Resume after crash: just re-run; checkpoints skip completed work.
# Fresh start: delete both checkpoint files.
#
# Usage:
#   Rscript scripts/run_nested_loso_v2.R
# ============================================================

cat("=== Optimized Nested LOSO M2 grid search (v2) ===\n")
...
# Runs ONCE. Result is reused across all 48 M2 specs.
# ============================================================
cat("=== PHASE 1: Folds + M1 predictions ===\n")
cat("(", length(test_seasons), "folds, ~10 seasons M1 each)\n\n")

phase1_ckpt <- "data/nested_loso_v2_phase1.rds"

if (file.exists(phase1_ckpt)) {
  m1_cache  <- readRDS(phase1_ckpt)
  remaining <- setdiff(test_seasons, names(m1_cache))
  cat("Resuming: ", length(m1_cache), " folds done, ", length(remaining), " remaining\n\n")
} else {
  m1_cache  <- list()
  remaining <- test_seasons
}

# Use multicore for M1 seasons within each fold
...
for (test_s in remaining) {
  ...
  # M1 on training seasons — parallelized across seasons via future::plan above
  ...
  # M1 on test season (single season, sequential)
...
# ============================================================
# PHASE 2: M2 grid search — M2 train + eval only (no M1 re-run)
# Parallelized across specs via furrr::future_map
# ============================================================
cat("=== PHASE 2: M2 grid search ===\n")
cat("(", length(specs), "specs x", length(test_seasons), "folds, M2 only)\n\n")

phase2_ckpt <- "data/nested_loso_v2_phase2.rds"
...
# Process specs in batches of n_cores for checkpoint granularity
batch_size <- n_cores
