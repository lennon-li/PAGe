...
# Governed exchangeable-season holdout cycle using the current PAGe stage API.
#
# The held-out season is an identifier, not a chronological boundary. Every
# season other than the explicit permanent exclusions and the holdout is included
...
repo_root <- Sys.getenv("PAGE_REPO_ROOT")
if (!nzchar(repo_root)) {
...
if (identical(Sys.getenv("PAGE_LOAD_MODE"), "installed")) {
...
holdout <- Sys.getenv("PAGE_HOLDOUT")
if (!grepl("^[0-9]{4}-[0-9]{2}$", holdout)) {
  stop("PAGE_HOLDOUT must be a season label such as 2012-13.")
}
...
run_id <- Sys.getenv("PAGE_RUN_ID", paste0(holdout, "-current-api-20260902"))
run_dir <- Sys.getenv("PAGE_RUN_DIR")
if (!nzchar(run_dir)) run_dir <- file.path(artifact_root, holdout, "runs", run_id)
preexisting <- if (dir.exists(run_dir)) {
  setdiff(list.files(run_dir, all.files = TRUE, no.. = TRUE),
          c("runner.pid", "watchdog.tsv"))
} else {
  character(0)
}
if (length(preexisting)) {
  stop("Refusing to overwrite non-empty output directory: ", run_dir)
}
...
if (preflight) {
  cat("2012-13 governed runner preflight OK\n")
...
message(holdout, " governed holdout replay complete: ",
