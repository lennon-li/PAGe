#!/usr/bin/env Rscript
# ============================================================
# Nested LOSO M2 Grid Search — v10 (targeted alpha boundary check)
#
# Purpose: Determine if alpha_state < 0.05 improves NLL.
#   v9 best spec had alpha=0.05 at the minimum tested value.
#   The trend was non-monotone (0.05 < 0.20 < 0.10), but
#   extending to {0.01, 0.02, 0.03} closes the boundary question.
#
# Grid: alpha_state {0.01, 0.02, 0.03, 0.05} x k_e {3, 4}
#   All other params fixed at v9 best: delta=0, Kr=1, k_f=5,
#   k_1=0, k_2=0, k_w=0, k_s=0.
#   k_e=2 in v9 was bumped to min~3 by mgcv; k_e=3 tests that
#   explicitly, k_e=4 confirms the direction.
#
# Output: data/nested_loso_v10_production.rds
# ============================================================

cat("=== Nested LOSO M2 grid search (v10 — alpha boundary check) ===\n")
...
# ---- 3. v10 grid: alpha boundary check ----
# Fixed at v9 best structure (delta=0, Kr=1, k_f=5, k_1=0, k_w=0, k_s=0)
# Vary: alpha_state (extend below 0.05) x k_e (3 is mgcv effective min, 4 for comparison)
grid_v10 <- tidyr::crossing(
  delta       = 0L,
  Kr          = 1L,
  k_f         = 5L,
  k_e         = c(3L, 4L),
  alpha_state = c(0.01, 0.02, 0.03, 0.05),
  k_1         = 0L,
  k_2         = 0L,
  k_w         = 0L,
  k_s         = 0L,
...
cat("Grid size:", nrow(grid_v10), "specs\n")
print(grid_v10)
cat("\n")

specs_v10 <- purrr::pmap(grid_v10, function(delta, Kr, k_f, k_e, alpha_state,
...
names(specs_v10) <- sprintf(
  "d%+d_Kr%d_kf%d_ke%d_a%.0f_k1%d_k2%d_kw%d_ks%d",
  grid_v10$delta, grid_v10$Kr, grid_v10$k_f, grid_v10$k_e,
  grid_v10$alpha_state * 100,
  grid_v10$k_1, grid_v10$k_2, grid_v10$k_w, grid_v10$k_s
)
cat("Full v10 grid:", length(specs_v10), "specs\n")
...
# ============================================================
# PHASE 1: Reuse existing M1 cache (M0+M1 unchanged)
# ============================================================
cat("=== PHASE 1: Loading M1 cache ===\n")

for (src in c(
  "data/nested_loso_v9_phase1.rds",
...
if (!exists("m1_cache")) stop("No Phase 1 cache found. Run v9 first.")
...
# ============================================================
# PHASE 2: Evaluate v10 specs
# ============================================================
cat("=== PHASE 2: M2 grid search (v10) ===\n")

new_spec_ids  <- names(specs_v10)
phase2_ckpt   <- "data/nested_loso_v10_phase2.rds"
...
# ============================================================
# Assemble v10 results, compare with v9 best
# ============================================================
cat("\n=== Assembling v10 results ===\n")

cv_results_all <- cv_results_new[names(specs_v10)]
...
best_spec <- specs_v10[[best_id]]

cat("\n=== v10 results (all specs, sorted by NLL) ===\n")
print(summary_df)

cat("\n=== Comparison with v9 best (NLL=40.143) ===\n")
v9_best_nll <- 40.14282
cat(sprintf("v9 best:  NLL=%.4f  (alpha=0.05, k_e=2→3)\n", v9_best_nll))
cat(sprintf("v10 best: NLL=%.4f  (spec: %s)\n", summary_df$mean_nll[1], best_id))
delta_nll <- v9_best_nll - summary_df$mean_nll[1]
cat(sprintf("Improvement: %.4f NLL  (%s)\n",
  delta_nll, if (delta_nll > 0.1) "meaningful" else "negligible"))

cat("\nBest spec params:\n")
cat("  delta=", best_spec$delta, " Kr=", best_spec$Kr,
    " k_f=", best_spec$k_f, " k_e=", best_spec$k_e,
    " alpha=", best_spec$alpha_state,
    " k_1=", best_spec$k_1, " k_w=", best_spec$k_w,
    " k_s=", best_spec$k_s, "\n")
...
results <- list(
  ...
  grid         = grid_v10,
  v9_best_nll  = v9_best_nll
)
saveRDS(results, "data/nested_loso_v10_production.rds")

cat("\nResults saved to data/nested_loso_v10_production.rds\n")
