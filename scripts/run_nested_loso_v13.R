#!/usr/bin/env Rscript
# ============================================================
# Nested LOSO M2 Grid Search — v13 (Phase1 clamping + Holt + R2 online RE)
#
# Changes from v12:
#   - Phase 1 parity fixes: feature_ranges clamping in LOSO (z_ema, logit_f_eff)
#   - Post-peak bias reset now present in LOSO (matches deployment)
#   - R1: Holt double-exponential bias (level + trend); bias_alpha & bias_beta tuned
#   - R2: Online season RE estimate (estimate_season_re_online) each week
#   - Grid: k_de fixed at 0 (k_de>0 catastrophically fails); expanded k_f, alpha_state,
#           k_r (includes 0 to ablate z_resid), bias_alpha x bias_beta as tunable dims
#   - Grid size: 4 x 2 x 5 x 3 x 4 x 2 = 960 specs
#   - Reuses M1 Phase 1 cache from v12
#
# Output: data/nested_loso_v13_phase2.rds (resumable checkpoint)
#         data/nested_loso_v13_production.rds (final)
# ============================================================

cat("=== Nested LOSO M2 grid search (v13 — Holt + R2 + Phase1 clamping) ===\n")
...
# ---- 3. v13 spec grid ----
# k_de fixed at 0 (k_de > 0 catastrophically fails at NLL 117+)
# bias_alpha and bias_beta are new tunable dimensions
grid_v13 <- tidyr::crossing(
  delta       = 0L,
  Kr          = 1L,
  k_f         = c(2L, 3L, 4L, 5L),
  k_e         = c(2L, 3L),
  ...
  k_r         = c(0L, 2L, 3L),
  k_de        = 0L,
  bias_alpha  = c(0.2, 0.3, 0.4, 0.5),
  bias_beta   = c(0.0, 0.1)
)

specs_v13 <- purrr::pmap(grid_v13, function(delta, Kr, k_f, k_e, alpha_state,
...
names(specs_v13) <- paste0(
  "d+", grid_v13$delta,
  "_Kr", grid_v13$Kr,
  "_kf", grid_v13$k_f,
  "_ke", grid_v13$k_e,
  "_as", grid_v13$alpha_state,
  "_kr", grid_v13$k_r,
  "_kde", grid_v13$k_de,
  "_ba", grid_v13$bias_alpha,
  "_bb", grid_v13$bias_beta
)

cat("Grid size:", length(specs_v13), "specs\n\n")

# ============================================================
# PHASE 1: Reuse existing M1 cache (M0+M1 are unchanged)
# ============================================================
cat("=== PHASE 1: Loading M1 cache ===\n")

for (src in c(
  "data/nested_loso_v13_phase1.rds",
  "data/nested_loso_v12_phase1.rds",
  "data/nested_loso_v11_phase1.rds",
...
# Save Phase 1 cache if not already saved as v13
p1_save <- "data/nested_loso_v13_phase1.rds"
...
# ============================================================
# SMOKE TEST — run 1 fold × 1 spec, then quit
# ============================================================
if (Sys.getenv("SMOKE_TEST", unset = "0") == "1") {
  cat("=== SMOKE TEST: 1 fold × 1 spec ===\n")
  test_s     <- test_seasons[1]
  fc         <- m1_cache[[test_s]]
  smoke_id   <- names(specs_v13)[1]
  smoke_spec <- specs_v13[[smoke_id]]
...
# ============================================================
# PHASE 2: Evaluate all v13 specs (frozen GAM + Holt + R2)
# ============================================================
cat("=== PHASE 2: M2 grid search (v13 — frozen + Holt + online RE) ===\n")

new_spec_ids <- names(specs_v13)
cat("Total v13 specs to evaluate:", length(new_spec_ids), "\n\n")

phase2_ckpt <- "data/nested_loso_v13_phase2.rds"
...
# ============================================================
# Assemble v13 results and report
# ============================================================
cat("\n=== Assembling v13 results ===\n")

cv_results_all <- cv_results_new[names(specs_v13)]
...
best_spec <- specs_v13[[best_id]]

cat("\n=== Top 20 specs by mean NLL ===\n")
print(utils::head(summary_df, 20), n = 20)
cat("\nBest spec:", best_id, "\n")
...
saveRDS(results, "data/nested_loso_v13_production.rds")

cat("\nResults saved to data/nested_loso_v13_production.rds\n")
