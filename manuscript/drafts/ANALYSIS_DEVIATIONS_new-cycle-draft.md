...
| D-32 | M1 hyperparameters fixed (`k_ref = 30`, `slope_weight = 16`) instead of tuned; prespecified sensitivity recipe `k_ref = 20`, `slope_weight = 12`, run on the four chronological Tier-1 seasons only (2022-23 to 2025-26), reported in the supplement (Lennon, 2026-09-16); fully nested gate (`gate_nesting = "full"`, validation default) reselects every tuned upstream axis (here M0) and the full M2 selection within each season-excluded dataset; `"conditional"` is a package option not used for validation | Lennon, 2026-09-15 | Flat M1 surface (0.16-0.18 weeks over 25 specs), unstable `k_ref` ranking, M1 tuning 72-78% of wall time | Prior-cycle tuning tables, r5, all-season kit `05f78903` tuning (no new-cycle outer results) | M1, gate, compute |
| D-33 | `m2_subset_train()`'s deployed-fit rows are always built from the M0 detector's declaration (`m2_subset_prefix_declaration()`), never from `timing_truth`, even when a truth table is supplied and even though `m2_subset_tune()`'s spec-selection rows deliberately use truth timing; the asymmetry is intentional and kept, not resolved by matching one side to the other | Claude/Opus review, 2026-09-16 `[FREEZE: commit]` | Tune produces a discrete spec id (cheap to get slightly wrong); train produces the fitted `s(u)` surface actually served at runtime, which must match the timing distribution runtime will present (detector-based, since a live season has no truth table). Fitting train on truth-`u` would calibrate the smooth and `feature_ranges` against a distribution the model never sees in production, and can also change the deployed training row population via the availability check `ew < dec$week`. M0 is explicitly tuned to declare early relative to the manual label (D-6), so this is a systematic, not zero-mean, skew | Liz (Codex) independent re-review of commit `02a900e`, verified against code, Opus adjudication 2026-09-16; no repo artifact yet quantifies detector-vs-truth ignition disagreement in weeks | M2 fit vs. tuning; deployment kit `declaration_provenance` |
