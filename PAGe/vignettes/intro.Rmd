...
```{r data-and-training, eval=FALSE, purl=FALSE}
library(PAGe)
...
For explicit training gates, declare disjoint season sets and complete each
stage before starting the next:

```{r guarded-stages, eval=FALSE, purl=FALSE}
...
A downstream stage rejects a draft or provenance-mismatched upstream artifact.
`train_pipeline()` remains the high-level compatibility orchestrator and has
not yet been refactored to compose these stage contracts.

## Holdout replay and promotion

```{r holdout-replay, eval=FALSE, purl=FALSE}
...
degradation, and at most 10% phase-MAE degradation. This in-memory comparison
is diagnostic only. It has no artifact provenance and must not be passed to
`train_pipeline()` to release the holdout. A governed release uses
`scripts/acceptance/replay_2025_26.R`, preserves its decision bundle and
manifest, performs the fixed-spec refresh with
`season2526/run_retrain_venkata.R`, and registers the result immutably with
...
```{r weekly-forecast, eval=FALSE, purl=FALSE}
...
* [Pipeline overview](articles/pipeline-overview.html) -- architecture and notation.
* [Pipeline walkthrough](articles/pipeline-walkthrough.html) -- an end-to-end training
  and deployment walkthrough.
* `?stage_contracts`, `?tune_m0`, `?tune_m1`, `?assemble_kit`,
  `?run_pipeline` -- reference pages for the guarded and high-level APIs.
