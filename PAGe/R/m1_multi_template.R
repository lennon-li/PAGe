...
#'     \item{spread_method}{Character; the spread method used.}
#'     \item{spread_fallback_count}{Integer; number of forecast weeks that fell
#'       back to between-only because per-template SEs were unavailable
#'       (total mode only; 0 for between mode).}
#'   }
align_multi_template <- function(currentD,
  ...
  if (sum(valid & is.finite(nlls)) == 0) {
    ...
    fb <- align_forecast_pipeline_dilate(
    ...
    fb$spread_method <- spread_method
    fb$spread_fallback_count <- 0L
    return(fb)
  }
  ...
  if (n_fw > 0) {
    ...
    # "between" = weighted between-template variance only (incumbent default).
    # "total"   = between-template variance + weighted per-template GAM SE^2.
    se_mat <- NULL
    if (spread_method == "total") {
      se_mat <- matrix(NA_real_, nrow = n_fw, ncol = length(valid_idx))
      for (j in seq_along(valid_idx)) {
        vi <- valid_idx[j]
        se_mat[, j] <- template_funs[[vi]]$g_ref_mu_se(fw_weeks)$se
      }
    }
    spread_res <- .compute_logit_spread(
      logit_mat, wts, se_mat, spread_method
    )
    logit_spread_ens <- spread_res$spread
    spread_fallback_count <- spread_res$fallback_count
  ...
  list(
    ...
    spread_fallback_count = spread_fallback_count
  )
}
...
#' is incremented.
#'
#' @param logit_mat Numeric matrix (n_future x n_templates) of logit-scale
#'   template forecasts.
#' @param wts Numeric vector of ensemble weights (length n_templates).
#' @param se_mat Numeric matrix or NULL; per-template GAM SEs at forecast weeks
#'   (same shape as \code{logit_mat}). Required for \code{"total"} mode.
#' @param spread_method Character; \code{"between"} or \code{"total"}.
#'
#' @return List with \code{spread} (numeric vector, length \code{nrow(logit_mat)})
#'   and \code{fallback_count} (integer).
#' @keywords internal
.compute_logit_spread <- function(logit_mat, wts, se_mat = NULL,
                                  spread_method = "between") {
  spread_method <- match.arg(spread_method, c("between", "total"))
  n_fw <- nrow(logit_mat)
  if (n_fw == 0) {
    return(list(spread = numeric(0), fallback_count = 0L))
  }

  between_var <- vapply(seq_len(n_fw), function(i) {
    lv <- logit_mat[i, ]
    mu <- sum(wts * lv)
    sum(wts * (lv - mu)^2)
  }, numeric(1))

  if (spread_method != "total" || is.null(se_mat)) {
    return(list(spread = sqrt(between_var), fallback_count = 0L))
  }

  has_se <- vapply(seq_len(n_fw), function(i) {
    any(!is.na(se_mat[i, ]) & se_mat[i, ] > 0)
  }, logical(1))

  within_var <- vapply(seq_len(n_fw), function(i) {
    se_use <- se_mat[i, ]
    se_use[is.na(se_use) | se_use <= 0] <- 0
    sum(wts * se_use^2)
  }, numeric(1))
