...
plotRes <- function(res, seasonIndex = NULL, peakInfo = NULL, currentSeason = NULL) {
  ...
  p1 <- ggplot() +
    ...
    labs(
      ...
      subtitle = if (!any(is.na(res$peak$t_peak))) {
        ...
        "Peak CI unavailable yet (early/unstable delta)."
      },
  ...
  xD <- res$pred_df |>
    left_join(currentSeason |> select(date, newWeek = weekF)) |>
    ...
    mutate(
      ...
      week = ifelse(
        newWeek >= 1 & newWeek <= nW_true,
        newWeek + startWeek - 1L,
        NA_real_
      ),
  ...
  p2 <- xD |> ggplot(aes(x = date, y = p_hat)) +
    geom_line() +
  ...
  if (!is.null(seasonIndex)) {
    ...
    start <- xD |>
      filter(newWeek == start_week) |>
      pull(week)
    end <- xD |>
      filter(newWeek == end_week) |>
      pull(week)
