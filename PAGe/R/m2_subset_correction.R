...
m2_subset_prefix_declaration <- function(data, params, origin_week,
  ...
  out <- detector(prefix, params = params, start_week = start_week)
  declaration <- as.integer(out$ign_week_locked)
  if (!is.finite(declaration)) declaration <- NA_integer_
...
m2_subset_runtime_prediction <- function(kit, current_data, m1_result,
  ...
  for (pw in m1_result$per_week) {
    ...
    declaration <- if (!all_off) {
      m2_subset_prefix_declaration(prefix, kit$m0_params, ew)
    } else {
    ...
    for (h in c(1L, 2L)) {
      target <- ew + h
      target_new <- target - ap$iWeek_hat + kit$ref$anchorWeek
