...
Each week after ignition, $M_2$ is refit on historical data augmented with current-season rows. Hyperparameters are frozen; only GAM coefficients update. $M_2$ runs **after** $M_1$ each week — it requires the aligned covariates $M_1$ just produced.
...
Within each week: $M_1$ aligns → if `state == "aligning"`, $M_2$ refits and forecasts. Full implementation: [`prospective_deployment.html`](prospective_deployment.html).
...
| **7** | **Weekly: $M_1$ → $M_2$ refit → forecast** | **Online, every week** |
