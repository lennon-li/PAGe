...
**Locked configuration:** `k_ref=25`, `temperature=0.25`, `shift=0`, `slope_window=6`, `slope_weight=8` → Weibull peak MAE = 1.275 weeks. The slope-similarity bonus (weighting templates by matching current growth rate) is the primary driver of improvement from the 1.929 baseline.
...
**Tuned spec** (v15-postfix, 7200-spec nested LOSO, 10 seasons, Bernoulli NLL = 0.5959 pre-L2 / 0.5796 post-L2):
```
delta=0, Kr=1, k_f=5, k_e=2, k_r=0, k_de=0, k_sp=2
alpha_state=0.40, bias_alpha=0.5, bias_beta=0, lambda_w=0, w_floor=0.05
```
Mode: **frozen GAM** (pre-trained once on all historical seasons) + **Holt EMA bias correction** (level-only, β=0; `bias_alpha=0.5` tuned in LOSO, chosen at the lower edge of the flat-NLL plateau for robustness) + online season random effect, applied week-by-week at deployment. The v15-postfix loop fixes four evaluation bugs (Holt asymptote, `dz_ema` standardization, silent online-RE no-op, retrospective `iWeek` leakage in test fold) that biased earlier v15 numbers low. Under the corrected loop, `k_sp` becomes load-bearing and `k_r` drops to 0 — `z_resid` is noisier with prospective `iWeek` and alignment uncertainty now contributes.
