...
**Tuned spec** (v14/v15 confirmed, 1920-spec nested LOSO, 10 seasons, Bernoulli NLL = 0.406):
```
delta=0, Kr=1, k_f=4, k_e=2, k_r=2, k_de=0, k_sp=0
alpha_state=0.40, bias_alpha=0.2, bias_beta=0.0, lambda_w=0, w_floor=0.05
```
Mode: **frozen GAM** (pre-trained once on all historical seasons) + **Holt EMA bias correction** (α=0.2, level-only) + online season random effect, applied week-by-week at deployment. v15 confirmed that rate-of-change (`k_de`) and alignment-uncertainty (`k_sp`) terms provide no improvement.
