...
**Tuned spec** (v15 confirmed, 480-spec nested LOSO, 10 seasons, Bernoulli NLL = 0.406):
```
delta=0, Kr=1, k_f=4, k_e=2, k_r=2, k_de=0, k_sp=0
alpha_state=0.40, lambda_w=0, w_floor=0.05
```
Mode: **frozen GAM** (pre-trained once on all historical seasons) + **adaptive Holt EMA bias correction** (level-only, β=0; base α is a deployment parameter set per-season based on season characteristics — boosted to 0.7 after ≥2 consecutive same-sign residuals) + online season random effect, applied week-by-week at deployment. v15 confirmed that rate-of-change (`k_de`) and alignment-uncertainty (`k_sp`) terms provide no improvement over the v14 spec.
