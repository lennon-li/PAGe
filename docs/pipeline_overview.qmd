...
**Tuned spec** (v15-postfix + boundary expansion, 192-spec focused LOSO + 5 expansion rounds, 10 seasons, Bernoulli NLL = 0.5959 pre-L2 / 0.5796 post-L2 / 0.5763 post-expansion):
```
delta=0, Kr=1, k_f=6, k_e=2, k_r=0, k_de=0, k_sp=2
alpha_state=0.35, bias_alpha=1.0, bias_beta=0, lambda_w=0, w_floor=0.05
```
Mode: **frozen GAM** (EDF≈26, pre-trained once on all historical seasons) + **Holt EMA bias correction** (level-only, β=0; `bias_alpha=1.0` — boundary expansion showed monotone NLL improvement across the full 0.1–1.0 range; full single-step correction, no smoothing) + online season random effect, applied week-by-week at deployment. The v15-postfix loop fixes four evaluation bugs (Holt asymptote, `dz_ema` standardization, silent online-RE no-op, retrospective `iWeek` leakage in test fold) that biased earlier v15 numbers low. Under the corrected loop, `k_sp` becomes load-bearing and `k_r` drops to 0 — `z_resid` is noisier with prospective `iWeek` and alignment uncertainty now contributes.
