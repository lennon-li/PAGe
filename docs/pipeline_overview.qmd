...
Each week after ignition, $M_2$ predicts from the frozen production GAM (fit offline on all historical seasons before the season starts). Hyperparameters and coefficients are fixed; only the covariates — derived from $M_1$ each week — update. $M_2$ runs **after** $M_1$ and forecasts for any post-ignition state (`"aligning"` or `"post_peak"`).

**Key functions:** `run_m2_forecast()`, `load_prospective_kit()`
...
Within each week: $M_1$ aligns → for any post-ignition state, $M_2$ forecasts from the frozen production GAM. Full implementation: [`prospective_deployment.html`](prospective_deployment.html).
...
| **7** | **Weekly: $M_1$ → $M_2$ forecast (frozen production fit)** | **Online, every week** |
...
**Frozen production fit at deployment.** $M_2$ is trained once offline on all
historical seasons (using the best nested-LOSO spec) and the fitted GAM is
frozen. At runtime, $M_2$ prediction uses updated $M_1$ covariates each week
but does not refit. All modelling happens in aligned-week space ($w^a_{s,w}$)
— the GAM smooth terms provide flexibility beyond $M_1$'s coarse alignment.
The season random effect is excluded at prediction time (`s(season)` dropped)
so the frozen fit generalises to the unseen current season.
