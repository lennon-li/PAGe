...
**Deployed spec** (`v16_fresh`, nested-LOSO Bernoulli NLL = 0.4175):
```
...
is retained only as an explicit compatibility option and is not the validated
production default.
...
one-standard-error and Pareto selection are explicit alternatives. Optional
staged racing removes only candidates whose uncertainty intervals clearly lose,
and every survivor is still evaluated with full nested LOSO.

The 2025-26 season is first replayed as an unseen prospective holdout. It may
enter the refresh used for 2026-27 only after `check_promotion()` confirms at
least 2% NLL improvement, no more than 5% MAE degradation at any forecast
horizon, and no more than 10% MAE degradation in any epidemic phase. Failed or
